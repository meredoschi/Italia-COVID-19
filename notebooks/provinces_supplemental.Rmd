...
plot(individual_province_rates$dt, individual_province_rates$cubic_perc_rate)
  
 txt<-'COVID-19 total case progression'
  cubic_chart_title<-paste(selected_province_name,"province")
  cubic_rate_chart<-ggplot(data=individual_province_rates)+geom_line(aes(x=dt,y=cubic_perc_rate),color="purple")+geom_line(aes(x=dt,y=current_perc_rate),color="darkblue")+ggtitle(cubic_chart_title)+xlab(paste(txt,format(min(individual_province_rates$dt),"%d %b"),"-",format(max(individual_province_rates$dt),"%d %b %Y")))+ylab("current vs. 3 day average (shown in purple) % growth rate")  
