...
total_cases_by_province<-dplyr::select(provinces_data, dt, denominazione_provincia, denominazione_regione, totale_casi)
...
local_province_data<-total_cases_by_province[total_cases_by_province$province_name==local_province_name,]
num_days_recorded<-nrow(local_province_data)
local_province_data$total_cases_day_before<-head_na(local_province_data$total_cases,1)
local_province_data$total_cases_three_days_before<-head_na(local_province_data$total_cases,3)

local_province_data$current_rate<-(local_province_data$total_cases)/(local_province_data$total_cases_day_before)
local_province_data$current_perc_rate<-(local_province_data$current_rate-1)*100
local_province_data$growth_factor_in_three_days<-((local_province_data$total_cases)/(local_province_data$total_cases_three_days_before))
local_province_data$cubic_rate<-'^'(local_province_data$growth_factor_in_three_days,1/3)
local_province_data$cubic_perc_rate<-(local_province_data$cubic_rate-1)*100
local_province_rates<-local_province_data[is.finite(local_province_data$cubic_perc_rate),]
  

  
  province_chart<-provincial_cubic_rate_chart(local_province_rates,local_province_name)
...
*This page last updated: 20/4/2020*
