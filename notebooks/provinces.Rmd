...
dpc_provinces <-
  dpc_province_names(provinces_data) # makes use of helper function
  province_names_to_adjust <-
  setdiff(dpc_provinces, as.character(istat_provinces$province_name))
  
  provinces_data$denominazione_provincia <-
  as.character(provinces_data$denominazione_provincia)
  ...
  provinces_data$denominazione_provincia)
  provinces_data$denominazione_provincia[record_to_change_indx] <-
  as.character(formal_province_name) # from the ISTAT dataset
  
  }
  provinces_data$denominazione_provincia <-
  as.factor(provinces_data$denominazione_provincia)
...
round((
provinces_calculations$current_rate - provinces_calculations$cubic_rate
) * 100,
digits = 1
)
provinces_calculations$per_capita <-
...
provinces_per_capita <-
  ...
  nrow(provinces_per_capita_latest) - rank(provinces_per_capita_latest$per_capita) +
  1
  provinces_per_capita_latest <-
  arrange(provinces_per_capita_latest,-per_capita)
  print(
  select(
  provinces_per_capita_latest,
  pos,
