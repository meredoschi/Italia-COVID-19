...
num_days_trailing_on_dt <-
  function(df,
           specified_dt,
           positives_remaining_col_name,
           new_positives_col_name) {
    df <-
      arrange(df, dt) # Important: it is assumed df has a date column called 'dt'
    
    df <- df[df$dt <= specified_dt,] # trim to specified dt
    
    df_specified_dt <- df[df$dt == specified_dt,]
    
    positives_remaining_col_indx <-
      grep(paste("^", positives_remaining_col_name, "$", sep = ''),
           colnames(df))
    new_positives_col_indx <-
      grep(paste("^", new_positives_col_name, "$", sep = ''),
           colnames(df))
    
    num_positives_remaining <-
      df_specified_dt[, positives_remaining_col_indx]
    
    i <- 0
    positives_detected_subtotal <- 0
...
#      print(new_positives_detected_on_dt)
    ...
    #  print(paste("positives_remaining =",num_positives_remaining))
    #  print(paste("positives_detected_subtotal =",positives_detected_subtotal))
    #  print(paste("Num days backlog =",i))
...
provinces_regional_territory_code_fix <- function(df) {
  df$regional_territory_code <-
    as.character(df$regional_territory_code)
  ...
  for (indx in trento_or_bolzano_indices) {
    df[indx,]$regional_territory_code <-
      trentino_sudtirol_territory_code
...
expand_dt_columns <-
  function(df,
           ...
           attrib_column_name) {
    recorded_dates <-
      as.character(uniq_dates(df)) # This cast is needed!
...
calculate_delta <- function(df, attrib, start_dt, finish_dt) {
  ...
  finish_dt_column_name <-
    append_dt_to_col_name(attrib, finish_date)
  
  #  num_days<-finish_date-start_date
  #  print(num_days)
  expanded_columns <-
    select(df,
           all_of(start_dt_column_name),
           all_of(finish_dt_column_name))
  start_dt_value <- expanded_columns[1, 1] # First row, first column
  finish_dt_value <-
    expanded_columns[1, 2] # First row, second column with all equal values, by definition, so we just pick the first one.
...
retrieve_attrib_value_on_dt <-
  function(df,
           specified_dt,
           nation_or_province_column_name,
           attrib_column_name) {
    attrib_on_dt_txt <-
      paste(attrib_column_name, as.character(specified_dt), sep = "_")
    ...
    revised_col_names <-
      gsub(attrib_column_name, attrib_on_dt_txt, col_names)
...
population_intl_column_names <- function(df) {
  ...
  intl_col_names <-
    gsub("Territory", "territory_name", intl_col_names)
...
retrieve_istat_zones <- function(istat_df) {
  istat_zones <-
    istat_population[nchar(as.character(istat_df$territory_code)) == 3,]
  istat_zones_ordered_by_code <-
    arrange(istat_zones, territory_code)
  istat_zones_ordered_by_code
}
...
retrieve_istat_regions <- function(istat_df) {
  istat_regions <-
    istat_df[nchar(as.character(istat_df$territory_code)) == 4,]
  istat_regions <- arrange(istat_regions, territory_name)
  # Filter autonomous provinces
  autonomous_provinces_indices <-
    grep("Provincia Autonoma", istat_regions$territory_name)
...
retrieve_istat_provinces <- function(istat_df) {
  istat_provinces <-
    istat_df[nchar(as.character(istat_df$territory_code)) == 5,]
  col_names <- colnames(istat_provinces)
  intl_col_names <-
    gsub("territory_name", "province_name", col_names)
...
translated_national_column_names <- function(df) {
  col_names <- colnames(df)
  intl_col_names <-
    gsub("ricoverati_con_sintomi",
         "hospitalized_with_symptoms",
         col_names)
  intl_col_names <-
    gsub("terapia_intensiva", "intensive_care", intl_col_names)
  intl_col_names <-
    gsub("totale_ospedalizzati",
         "hospitalized_total",
         intl_col_names)
  intl_col_names <-
    gsub("isolamento_domiciliare", "home_isolation", intl_col_names)
  intl_col_names <-
    gsub("variazione_totale_positivi",
         "num_positives_variation",
         intl_col_names)
  intl_col_names <-
    gsub("totale_positivi", "positives_remaining", intl_col_names)
  intl_col_names <-
    gsub("nuovi_positivi", "new_positives", intl_col_names)
  intl_col_names <-
    gsub("dimessi_guariti",
         "total_discharged_healed",
         intl_col_names)
  intl_col_names <-
    gsub("deceduti", "total_deceased", intl_col_names)
  intl_col_names <-
    gsub("totale_casi", "total_cases", intl_col_names)
  ...
  # Added for provinces
  intl_col_names <-
    gsub("denominazione_provincia", "province_name", intl_col_names)
  colnames(df) <- intl_col_names
  df
...
revised_province_column_names <- function(df) {
  ...
  revised_col_names <-
    gsub("current_perc_rate", "daily_growth", col_names)
  revised_col_names <-
    gsub("cubic_perc_rate", "three_day_rate", revised_col_names)
  revised_col_names <-
    gsub("province_name", "province", revised_col_names)
  revised_col_names <-
    gsub("per_capita_rate", "per_capita", revised_col_names)
...
add_three_and_seven_day_rate_columns <- function(df, attrib) {
  ...
  df$growth_factor_in_three_days <-
    (attrib_col) / (df$attrib_three_days_before)
  ...
  df$growth_factor_in_seven_days <-
    (attrib_col) / (df$attrib_seven_days_before)
