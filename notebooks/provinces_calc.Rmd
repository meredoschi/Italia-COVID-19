...
datasets_path<-file.path(getwd(),"..", "Protezione-Civile-Dataset") 
```
...
italian_adm_regions_csv<-italian_adm_regions 
italian_adm_regions_to_fix<-italian_adm_regions[italian_adm_regions$region_code!=4,]
trento_bolzano_region<-data.frame('Trentino - Alto Adige | Trentino - Südtirol',4)
names(trento_bolzano_region)<-c("region_name","region_code")
italian_adm_regions<-full_join(italian_adm_regions_to_fix, trento_bolzano_region)
...
# Fix
italian_adm_provinces$region_name<-NULL
italian_adm_provinces<-inner_join(italian_adm_provinces, italian_adm_regions)
...
### Graph - Total case progression - subnational level (current vs. last three days rate)

#### Provinces by Region 

```{r echo=FALSE, message=FALSE}
provinces_calc$current_perc_rate<-(provinces_calc$current_rate-1)*100
require(ggplot2)
...
for (i in 1:nrow(italian_adm_regions) ) {
    ...
    local_province_data$cubic_perc_rate<-(local_province_data$cubic_rate-1)*100
    
    province_label_text<-element_text(face = "bold", color = "black", size = 11, angle=20)
    chart_title<-paste(specified_region_name,'(',format(three_days_before,"%d %b"),"->",format(most_recent_dt,"%d %b %Y"),")")
       current_rate_chart<-ggplot(local_province_data, aes(y=cubic_perc_rate, x=province_name))+geom_col(color="yellow", fill='#401030')+geom_boxplot(aes(y=current_perc_rate, x=province_name),color="cyan")+ggtitle(chart_title)+xlab("Province")  +theme(axis.text.x = province_label_text) +
 ylab(paste("Total cases % change (current daily vs. weighted 3 day rate)"))
       
       print(current_rate_chart)
...
```{r echo=FALSE, message=FALSE}
print(paste("Data last updated: ",format(most_recent_dt,"%d %b %Y")))
```
