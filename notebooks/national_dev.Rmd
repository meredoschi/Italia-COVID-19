---
title: "National calculations and charts - Italy - COVID-19 Epidemic (development version - draft)"
...
indicators<-c("total_cases","intensive_care","hospitalized_with_symptoms","home_isolation","total_discharged_healed","total_deceased","tests","positives_remaining")

for (i in 1:length(indicators)) { 
  
national_data<-expand_dt_columns(national_data, "nation", indicators[i])
}  

write.csv(national_data,file.path(csv_exports_dir,"national_data.csv"))
...
national_deltas<-select(national[-1,],dt,all_of(indicators))
  ...
  for (i in 1:num_days) {
      ...
      national_deltas[i,1+j]<-calculate_delta(national_data,indicators[j],start_dt,finish_dt) 
  
  }
    
  }

  national_deltas$hospitalized_total<-national_deltas$hospitalized_with_symptoms+national_deltas$intensive_care
...
```{r echo=FALSE, message=FALSE}
national_calculations<-inner_join(national, national_deltas)
national_calculations$ratio_tests_per_positive_added<-national_calculations$tests_delta/national_calculations$positives_added
# To do: work on national_calculations 
```
...
```{r echo=FALSE}
      nat_fatalities<-select(nat3,total_deceased, total_cases, positives_added_on_dt, dt, positives_remaining_delta)
      nat_fatalities$rate<-nat_fatalities$total_deceased/nat_fatalities$total_cases*100
    nat_fatalities<-select(nat_fatalities,total_deceased, total_cases, rate, dt, positives_added_on_dt, positives_remaining_delta)
    col_names<-colnames(nat_fatalities)
    intl_col_names<-gsub("positives_added_on_dt","cases_added",col_names)
...
```{r echo=FALSE, message=FALSE}
write.csv(nat3,file.path(csv_exports_dir,"nat3.csv"))
```
