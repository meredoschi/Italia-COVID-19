...
dpc_covid19_ita_national<-read.csv(national_csv_fname,encoding="UTF-8", na.strings="undefined") 
national<-dpc_covid19_ita_national
```

```{r echo=FALSE, message=FALSE}
national<-translated_column_names(dpc_covid19_ita_national)
...
deltas_nat<-select(national[-1,],dt,all_of(indicators))



  for (i in 1:num_days) {

      start_dt<-earliest_dt+i-1 
      finish_dt<-start_dt+1 
      
      for (j in 1:length(indicators)) { 
      deltas_nat[i,1+j]<-calculate_delta(nat_expanded,indicators[j],start_dt,finish_dt) 
  
  }
    
  }  

  deltas_nat$hospitalized_total<-deltas_nat$hospitalized_with_symptoms+deltas_nat$intensive_care

  df_col_names<-colnames(deltas_nat)
  
  column_indices_to_mark_as_delta<-grep("dt",df_col_names,invert=TRUE) # i.e. excluding dt 

  column_names_to_mark_as_delta<-colnames(deltas_nat)[column_indices_to_mark_as_delta]
  ...
  for (i in 1:length(column_names_to_mark_as_delta)) {
      ...
      df_col_names<-gsub(original_column_name,delta_column_name,df_col_names)
...
nat_calc$tests_per_new_positives<-nat_calc$tests_delta/nat_calc$new_positives
nat_calc$total_tests_per_cases_ratio<-nat_calc$tests/nat_calc$total_cases
```

```{r echo=FALSE, message=FALSE}

nat_calc$positives_remaining_previous<-nat_calc$positives_remaining-nat_calc$positives_remaining_delta
#identical(nat_calc$positives_remaining_delta, nat_calc$num_positives_variation)
nat_calc$positives_remaining_percent_delta<-nat_calc$positives_remaining_delta/nat_calc$positives_remaining_previous*100
nat_calc$deceased_per_positives_day_before_ratio<-nat_calc$total_deceased_delta/nat_calc$positives_remaining_previous*100
nat_calc$discharged_healed_per_positives_day_before_ratio<-nat_calc$total_discharged_healed_delta/nat_calc$positives_remaining_previous*100

```

```{r echo=FALSE}
      nat_fatalities<-select(nat_calc,total_deceased, total_cases, new_positives, dt, positives_remaining_delta)
      nat_fatalities$rate<-nat_fatalities$total_deceased/nat_fatalities$total_cases*100
    nat_fatalities<-select(nat_fatalities,total_deceased, total_cases, rate, dt, new_positives, positives_remaining_delta)
    col_names<-colnames(nat_fatalities)
    intl_col_names<-gsub("new_positives","cases_added",col_names)
    intl_col_names<-gsub("positives_remaining_delta","net_effect",intl_col_names)
...
```{r fig.height=5, fig.width=10, echo=FALSE, message=FALSE}
    ...
    chart3<-ggplot(nat_calc, aes(y=deceased_per_positives_day_before_ratio, x=dt))+geom_path(color="orange")+ggtitle(chart3_title)+xlab("Date")+
 ylab("In thousands")
    print(chart3)
...
```{r echo=FALSE, message=FALSE}
nat4<-select(nat_calc, dt, positives_remaining_previous, total_deceased_delta, total_discharged_healed_delta, deceased_per_positives_day_before_ratio, discharged_healed_per_positives_day_before_ratio)
...
```{r fig.height=5, fig.width=10, echo=FALSE, message=FALSE}
    ...
    chart4<-ggplot(nat4, aes(y=deceased_per_positives_day_before_ratio, x=dt))+geom_line(color="red",linetype=2)+ggtitle(chart4_title)+xlab(paste(format(min(nat4$dt),"%d %b"),"-",format(max(nat4$dt),"%d %b %Y")))+
 ylab("%")
    print(chart4)
...
```{r fig.height=5, fig.width=10, echo=FALSE, message=FALSE}
    ...
    chart6<-ggplot(nat4, aes(y=positives_remaining_previous/total_discharged_healed_delta, x=dt))+geom_line(color="purple",linetype=5)+ggtitle(chart6_title)+xlab(paste(format(min(national$dt)+1,"%d %b"),"-",format(max(national$dt),"%d %b %Y")))+ylab("Proportion (smaller numbers are better)")
    print(chart6)
```
...
```{r fig.height=5, fig.width=10, echo=FALSE, message=FALSE}
    ...
    chart7<-ggplot(nat4, aes(y=total_discharged_healed_delta/total_deceased_delta, x=dt))+geom_line(color="#7D2860")+ggtitle(chart7_title)+xlab("Date")+ylab("Higher figures are better")
    print(chart7)
```
...
```{r echo=FALSE, message=FALSE}
nat5<-select(nat_calc, dt, total_discharged_healed, total_deceased, positives_remaining, total_cases)
...
```
*This page last updated: 18/4/2020*
