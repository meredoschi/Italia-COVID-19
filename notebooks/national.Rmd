...
# national<-national[national$dt>='2021-7-1',] # (optional, to restrict dates)
...
```{r echo=FALSE}  
  nat_fatalities$dt<-format(nat_fatalities$dt,'%d/%m/%Y')  
...
```{r fig.height=5, fig.width=10, echo=FALSE, message=FALSE, warning=FALSE}
chart8_title <-
  paste('Home isolation daily changes',
  format(min(national$dt), "%d %b %Y"),
  "-",
  format(max(national$dt), "%d %b %Y")
  )
  ...
  chart8 <-
  ggplot(data=deltas_nat_labelled) + geom_line(aes(y = home_isolation_delta, x = dt),color = "orange")  +   ggtitle(chart8_title) + xlab("Date") + ylab("people")
...
chart9_revised_title <-
  ...
  geom_line(aes(y = days, x = dt),color = "darkblue") + ggtitle(chart9_revised_title) + xlab(paste(format(min(nat_trailing$dt), "%d %b %Y"),
  "-",
