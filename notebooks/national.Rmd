...
```{r echo=FALSE, message=FALSE}
    ...
    chart0_title<-paste('Remaining active cases (daily change)')
    
    chart0<-ggplot(nat3, aes(y=deceased_per_positives_day_before_ratio, x=dt))+geom_path(color="orange")+ggtitle(chart0_title)+xlab("Date")+
 ylab("In thousands")
    print(chart0)
...
```{r echo=FALSE, message=FALSE}
    ...
    chart1_title<-paste('Fatalities per active cases on the previous day')
    
    chart1<-ggplot(nat4, aes(y=deceased_per_positives_day_before_ratio, x=dt))+geom_line(color="red")+ggtitle(chart1_title)+xlab(paste(format(min(nat$dt),"%d %b"),"-",format(max(nat$dt),"%d %b %Y")))+
 ylab("%")
    print(chart1)
    
    chart2_title<-paste('Discharged or healed per positive cases on the previous day')
    
    chart2<-ggplot(nat4, aes(y=discharged_healed_per_positives_day_before_ratio, x=dt))+geom_line(color="green")+ggtitle(chart2_title)+xlab(paste("Most recent observation:",format(max(nat$dt),"%d %b %Y")))+
 ylab("times (proportion)")
    print(chart2)
```
...
```{r echo=FALSE, message=FALSE}
    ...
    chart3_title<-paste('Total active cases on the day before per each healed or discharged "today"')
    
    chart3<-ggplot(nat4, aes(y=positives_remaining_previous/discharged_healed_delta, x=dt))+geom_line(color="purple")+ggtitle(chart3_title)+xlab(paste("Most recent observation:",format(max(nat$dt),"%d %b %Y")))+
...
```{r echo=FALSE, message=FALSE}
      chart5_title<-paste('Discharged or healed per fatality proportion')
    
    chart5<-ggplot(nat4, aes(y=discharged_healed_delta/deceased_delta, x=dt))+geom_line(color="brown")+ggtitle(chart5_title)+xlab(paste("Most recent observation:",format(max(nat$dt),"%d %b %Y")))+
 ylab("Higher figures are better")
    print(chart5)
```
