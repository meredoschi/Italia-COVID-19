...
datasets_path <-
file.path(getwd(), "..", "Protezione-Civile-Dataset", "COVID-19")
...
chart7_title <-
  paste(
  'Discharged or healed per fatality proportion',
  format(min(national$dt), "%d %b"),
  "-",
  format(max(national$dt), "%d %b %Y")
  )
  ...
  chart7 <-
  ggplot(nat4,
  aes(y = total_discharged_healed_delta / total_deceased_delta, x = dt)) +
  geom_line(color = "#7D2860") + ggtitle(chart7_title) + xlab("Date") + ylab("Higher figures are better")
  print(chart7)
```
...
national_case_backlog <-
nat6[nat6$inactive_discharged_healed_deceased >= initial_num_cases, ]
```

```{r echo=FALSE, message=FALSE}
national_case_backlog$positives_remaining_days_trailing_ratio<-national_case_backlog$positives_remaining/as.integer(national_case_backlog$num_days_trailing)
```

```{r echo=FALSE, message=FALSE}
chart8_title <-
  paste("'Backlog' - inactive (fatalities, discharged or healed) vs. cases still active")
  
  chart8 <-
  ggplot(data=national_case_backlog)+
  geom_line(aes(y = num_days_trailing, x = dt),color = "darkblue") + ggtitle(chart8_title) + xlab(paste(format(min(national_case_backlog$dt), "%d %b"),
  "-",
  format(max(national_case_backlog$dt), "%d %b %Y"))) + ylab("Approximate number of days behind")
  print(chart8)
```
